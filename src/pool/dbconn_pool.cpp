...
DBConnPool* DBConnPool::getDBConnPool()                            // 单例模式，得到当前的唯一实例
{
...
DBConnPool::DBConnPool()
{
    if(!parseJsonFile()) {                                         // 加载配置文件
        Error("The mysql config info load failed !");
    }
...
DBConnPool::~DBConnPool()
{
    while(!m_dbConnQueue.empty()) {                                // 队列不空时，将队列中的所有连接依次释放
    ...
    while(true) {
        ...
        while(m_dbConnQueue.size() >= m_minConnSize) {      // 检测非活跃连接同时维持一个最小连接数
            ...
            if(conn->getConnAliveTime() >= m_maxFreeTime) { // 判断是否超过了空闲时长
                m_dbConnQueue.pop();
                delete conn;
                Info("Inactive database connection disconnected !");
            }
            else {
                break;
            }
